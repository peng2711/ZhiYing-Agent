...
ECHOMIND_USER_ID_SECRET=请替换为随机高强度值
...
ECHOMIND_LLM_TIMEOUT_S=45
ECHOMIND_MAX_TOOL_ROUNDS=3
ECHOMIND_CORS_ORIGINS=http://localhost:3000,http://localhost:5173
ECHOMIND_ALLOW_CLIENT_USER_ID=false
EVAL_AUTO_PROMOTE_BASELINE=false
```

### 2.4 匿名用户与记忆隔离

`/chat` 不要求业务用户登录。首次访问时，服务端通过 HttpOnly Cookie 签发独立的
`guest_id`，后续请求自动使用该 ID 隔离 Redis 工作记忆。访客只保留当前会话的短期记忆；
接入真实认证后的用户才会写入/读取 ChromaDB 情景记忆和用户画像。

`ChatRequest.user_id` 仅为旧客户端兼容字段，默认不会作为身份依据；只有明确设置
`ECHOMIND_ALLOW_CLIENT_USER_ID=true` 才启用旧行为，生产环境不建议打开。

如果已有统一认证网关，可让网关传入 `X-Authenticated-User`，并使用
`ECHOMIND_USER_ID_SECRET` 对用户 ID 做 HMAC-SHA256，签名放在
`X-Authenticated-User-Signature`。EchoMind 验签成功后才会把该 ID 用于长期记忆；不能只传
明文用户 ID，否则请求会被拒绝或按访客处理。
...
|------|------|------|
| `message` | 是 | 用户输入 |
| `user_id` | 否 | 旧客户端兼容字段；默认不作为身份依据，服务端使用 `guest_id` Cookie |
| `conv_id` | 否 | 会话 ID，不传则自动生成 |

未登录用户首次调用 `/chat` 时，服务端会返回 `Set-Cookie: echomind_guest_id=...`。
浏览器应携带 Cookie 发起后续请求；访客只保存短期记忆，认证用户才会启用长期记忆。
...
| `tools_attempted` | Agent 请求调用过的工具（包括失败或降级调用） |
| `tools_used` | 成功返回的工具调用；不代表 fallback 成功 |
| `routing_reason` | 路由原因，包含意图、意图组、主辅 Agent 和领域分数 |
| `routing_confidence` | 主 Agent 路由分数 |
| `escalated` | 是否触发升级 |
| `latency_ms` | 端到端耗时 |
| `knowledge_used` | `search_knowledge_base` 成功返回过结果时为 true |
...
```json
{
  ...
  "tools_attempted": ["search_knowledge_base"],
...
|------|------|
| `message` | 用户输入 |
| `user_id` | 旧客户端字段；默认使用服务端签发的 `guest_id`，登录态应由认证 Token 决定 |
...
| `tools_attempted` | 尝试调用的工具 |
| `tools_used` | 成功调用的工具 |
| `routing_reason` | 路由原因和领域分数 |
| `routing_confidence` | 主 Agent 路由分数 |
| `escalated` | 是否触发升级/转人工 |
| `latency_ms` | 端到端延迟 |
| `knowledge_used` | 是否成功使用了 RAG 工具结果 |
| `entities` | 结构化实体 |
...
多轮对话需要保持同一个 `conv_id`，并让客户端持续携带服务端签发的
`echomind_guest_id` Cookie。匿名访客只使用 Redis 短期记忆，认证用户才会使用长期记忆。
