...
用户请求
  ...
  -> IntentRecognizer 三路融合识别细粒度意图、意图组和实体
  -> 按意图决定是否调用 MCPToolManager 检索 knowledge_base
...
请求进入 Orchestrator
    ↓
1. 特殊路由
   CRITICAL / HUMAN_HANDOFF -> 升级
    ↓
2. 领域打分
   TECHNICAL/TECHNICAL_LOGIN/TECHNICAL_CRASH -> TechnicalAgent
   BILLING/REFUND/INVOICE/PAYMENT_ISSUE/ACCOUNT_SECURITY -> BillingAgent
   QUERY/ORDER_STATUS/LOGISTICS/REQUEST 等 -> GeneralAgent
    ↓
3. 生成 RoutingDecision
   primary_agent = 最高分 Agent
   supporting_agents = 达到阈值的辅助专业 Agent
    ↓
4. 性能路由
...
当前 `run()` 主链路已经接入结构化复合问题检测，不再只是手动调用 `run_parallel()`，也不是简单关键词命中后无主次地拼接。系统会先计算 `general`、`technical`、`billing` 三个领域分数，再决定主 Agent 和辅助 Agent。
